        ...
        for i in data["alerts"]:
            try:
                ...
                if "message" in i["annotations"]:
                    message = message + i["annotations"]["description"]
                if "summary" in i["annotations"]:
                    summary = summary + i["annotations"]["summary"]
                if "namespace" in i["labels"]:
                    namespace = namespace + i["labels"]["namespace"]
                if "pod" in i["labels"]:
                    pod = pod + i["labels"]["pod"]
                if "container" in i["labels"]:
                    container = container + i["labels"]["container"]  
                if "node_name" in i["labels"]:
                    node_name = node_name + i["labels"]["node_name"]                   
                #if i["startsAt"]:
                ...
                #alert = cluster + "\n" + alertname + "\n" + severity + "\n" + instance + "\n" + message + "\n" + start + "\n" + end
                alert =  alertname + "\n" + severity + "\n" + instance + "\n" + message + "\n" + namespace + "\n" + pod + "\n" + container + "\n" + summary 
                mongo_alert = alertname + "\n" + severity + "\n" + node_name + "\n" + summary + "\n" + message 

                if  (len(node_name) == 0):
                    webex = [("roomId", webex_room), ("markdown", str(alert))]
                else:
                    webex = [("roomId", webex_room), ("markdown", str(mongo_alert))]
